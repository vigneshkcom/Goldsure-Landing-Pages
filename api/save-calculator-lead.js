...
async function sendInternalEmail(lead) {
  ...
  var html = [
    ...
    '<p style="margin:0;font-size:11px;color:#6b7899;">' + escapeHtml(new Date(lead.created_at || Date.now()).toISOString()) + '</p>',
    '<p style="margin:4px 0 0;font-size:11px;color:#6b7899;">Tracker: <strong style="color:#141c2e;">' + escapeHtml(lead.page_path || '-') + '</strong></p>',
    '</td></tr></table>',
